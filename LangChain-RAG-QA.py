...
import tempfile
import os
from langchain_classic.memory import ConversationBufferMemory # LangChain中最基础的记忆组件
...
# 检查API密钥（必须在所有OpenAI组件调用前执行）
if not openai_api_key:
    st.warning("请在侧边栏输入OpenAI API Key后继续")
    st.stop()
# ================================================================================
...
# 自动读取环境变量中的API Key（无需手动传参）
llm = ChatOpenAI(
    model="gpt-3.5-turbo",  # 适配问答场景的模型
    temperature=0  # 0表示回答更精准，无随机性
)
